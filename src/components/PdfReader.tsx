...
import { ArrowLeft, Sun, Moon, MoreVertical, Minimize, Maximize, RotateCcw, RotateCw, Bookmark, BookmarkCheck, BookmarkPlus } from 'lucide-react'
  ...
  const toggleBookmark = async () => {
    ...
    try {
      if (isBookmarked) {
        ...
        const note = `${t('reader.pageShort')} ${currentPage}/${numPages || 0} (%${percentage})`
        const id = await addBookmark(userId, bookId, loc, note, '')
        if (id) {
                ...
                {bookmarks.map((bm) => {
                  ...
                  return (
                    <button
                      ...
                      className="p-2 bg-white/60 dark:bg-dark-800/60 border border-white/30 dark:border-dark-700/30 rounded-lg text-left hover:bg-white/80 dark:hover:bg-dark-700/80"
                    >
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{t('reader.pageShort')} {page}/{numPages || 0}</div>
                      {bm.note && <div className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">{bm.note}</div>}
                    </button>
                  )
                })}
              </div>
